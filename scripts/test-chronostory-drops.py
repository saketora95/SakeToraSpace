...
class ImportTests(unittest.TestCase):
    def fixture(self):
        ...
        mage.update({"C4": "武器", "D4": "魔靈之翼", "E4": "68.0", "F4": "83.0", "G4": "83 攻",
                     "I4": "雪吉拉(0.21%)"})
        summary = {"B9": "神木村 後半", "C9": "赤翼龍", "D9": "弱冰\n抗火", "J9": "火槍攻擊 10%",
                   "B10": "神木村 後半", "C10": "墮落的翼龍", "E10": "閃耀的龍鱗戒指 (頂 265 HP + 1 捲)",
                   "B11": "冰原雪域", "C11": "雪吉拉戰車", "F11": "65 魔靈之翼 / 83"}
        return IMPORTER.compile_data({"武器卷軸": scrolls, "法師 (四速)": mage, "後期簡表": summary}, "2026-09-24", "test")
    ...
    def test_luk_requirement_is_not_level_and_similar_monsters_stay_distinct(self):
        data = self.fixture()
        item = next(i for i in data["items"] if i["name"] == "魔靈之翼")
        self.assertEqual(item["variants"][0]["requirement"], {"type": "luk", "value": 68})
...
class BundledDataTests(unittest.TestCase):
    ...
    def test_no_spreadsheet_locations_or_raw_cells_are_exported(self):
        ...
        self.assertEqual(allowed, IMPORTER.EQUIPMENT_SHEETS | IMPORTER.SCROLL_SHEETS | {"後期簡表"})
